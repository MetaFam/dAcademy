          ...
          >
            ...
            <CarouselContent>
              {category.books.map((book, bookIndex) => (
                <CarouselItem key={bookIndex} id={toSlug(book.title)} className="md:basis-1/2 lg:basis-1/4">
                  <Link to={`/book/${toSlug(book.title)}`} className="p-0 relative">
                    {!titleLess.includes(book.title) && (
                      <h3 className="text-xl font-semibold text-center absolute top-1/2 left-1/2 -translate-x-1/2 md:-translate-y-1/2 bg-black/30 p-2 max-sm:top-0">{book.title}</h3>
                    )}
                    <Card className="dark:bg-zinc-800">
                      <CardContent className="flex items-center justify-center p-0 dark:text-white">
                        <img className="object-cover rounded-r-lg border border-white" src={images?.[book.title]} alt={book.title} />
                      </CardContent>
                    </Card>
                  </Link>
                </CarouselItem>
